...
class TrackingEnv(gym.Env):
    ...
    def __init__(self, target_mode, obstacle_mode, test_flag=False, cl_flag=False, training_stage=1):
        ...
        self.observation_space = spaces.Box(low=-1, high=1, shape=(FUTURE_SIZE*2 + utils.HISTORY_SIZE*2,), dtype=np.float32) \
              if algo_config.use_god_view else spaces.Box(low=-1, high=1, shape=( utils.HISTORY_SIZE*2,), dtype=np.float32)
        # 初始化静态和动态障碍物列表
    ...
    def step(self, action):
        ...
        if self.target_mode == "Nav":
            ...
            current_target = self.target.copy()
            god_view_list = []
            
            # 生成未来预测数据
            for _ in range(FUTURE_SIZE):
                # 预测下一个目标位置（不实际移动）
                next_target = utils.see_move_clockwise(current_target, 
                                                    self.target_move_direction,
                                                    self.moving_size)
                
                # 使用utils的官方方法计算相对参数
                rel_distance, _, rel_angle, _, _ = utils.get_relative_elements(
                    next_target, self.tracker, self.angle)
                
                # 归一化处理
                norm_distance = np.clip(rel_distance/task_config.max_detection_distance, -1, 1)
                norm_angle = np.clip(rel_angle/task_config.max_detection_angle, -1, 1)
                
                god_view_list.extend([norm_distance, norm_angle])
                
                current_target = next_target  # 更新当前预测位置
            
            god_view_array = np.array(god_view_list, dtype=np.float32)
...
if __name__ == '__main__':
    '''主函数'''
    env = gym.make('Catching-v0', target_mode="Fix", obstacle_mode="None", test_flag=True)
    total_steps = 300
