...
def reward_calculate(test, last_target_distance, last_target_angle, continuous_tracking_step, loss_step, max_continuous_tracking_step, tracker, target, static_obstacles, dynamic_obstacles):
    ...
    # case3: 拦截成功
        ...
        distance_error = last_target_distance / task_config.max_detection_distance
        angle_error = abs(last_target_angle) / task_config.max_detection_angle
        reward += -1 * (distance_error + angle_error)
...
def get_relative_elements(target, tracker, angle):
    ...
    target_distance_error = (target_relative_distance - task_config.best_distance) \
                            / task_config.max_detection_distance  # 距离误差
    # 计算目标与智能体的相对角度及误差
...
def get_observation(last_target_distance, last_target_angle, last_seen_target_distance, last_seen_target_angle, tracker, angle, static_obstacles, dynamic_obstacles):
    """
    获取智能体的观测信息，并包含前几步的历史观测信息。如果历史数据缺失，用1填充。
    ...
    返回:
    - agent_observation: 智能体的观测信息，包含当前观测和前几步的历史观测信息，缺失部分用1填充。
    - last_seen_target_distance: 更新后的最后看到目标的距离。
    - last_seen_target_angle: 更新后的最后看到目标的角度。
    """
    global history_observations
    ...
    # 获取当前观测信息
    current_observation = np.array([distance, angle], dtype=np.float32)

    # 将当前观测信息添加到历史观测列表中
    history_observations.append(current_observation)

    # 如果历史观测列表的长度超过 HISTORY_SIZE，则移除最早的观测信息
    if len(history_observations) > HISTORY_SIZE:
        history_observations.pop(0)

    # 如果历史观测数据不足，用1填充缺失的部分
    while len(history_observations) < HISTORY_SIZE:
        history_observations.insert(0, np.array([1.0, 1.0], dtype=np.float32))

    # 将历史观测信息与当前观测信息合并
    agent_observation = np.hstack(history_observations)
