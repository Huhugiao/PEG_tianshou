...
mission = 0  # 可选值：
             # 0-训练tracker（target用规则）
             # 1-训练target（tracker用智能）
             # 2-训练tracker（target用智能）

task = "Protecting-v0"  # 环境名称，指定使用的环境类型
...
gamma = 0.99  # 折扣因子，用于计算未来奖励的当前价值，值越大代表更看重长期回报
...
step_per_epoch = 30000  # 每个训练周期内的步数，表示每个周期中进行的环境交互步数
...
test_num = 16  # 测试环境数量，表示并行测试时的环境数目
...
save_interval = 1  # 保存模型的间隔，每隔多少步或周期保存一次模型
repeat_per_collect = 4  # 每次数据收集后，重复训练的次数，控制训练更新频率

# 算法参数-PPO
vf_coef = 0.25  # 值函数损失的系数，用于平衡策略损失和值函数损失
ent_coef = 0.01  # 熵损失的系数，增加策略的随机性，防止过早收敛
...
epoch = 400 # 训练周期总数，表示模型训练的总迭代次数

wb_project = "protect_vs_invade"

wb_name = "_adjusting_34"
logdir = os.path.join("tblogs", wb_project + wb_name)
run_id = "16"


alt_wb_name = "INV"  # Name for alternate agent's logs
alt_logdir =  f"{wb_project}/{alt_wb_name}"  # Alternate log directory
alt_run_id = "i1"  # Run ID for alternate agent
