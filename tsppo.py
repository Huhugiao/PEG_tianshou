...
#     def forward(
#         self, obs: Union[np.ndarray, torch.Tensor], god_view: Optional[torch.Tensor] = None
#     ) -> torch.Tensor:
...
class PPOPolicy(A2CPolicy):
    ...
    def process_fn(
        ...
        # 处理 godview 信息
        if 'god_view_info' in batch.info:
            god_view = to_torch_as(batch.info['god_view_info'], batch.v_s)
            batch.data['god_view'] = god_view
        
        old_log_prob = []
        with torch.no_grad():
            for minibatch in batch.split(self._batch, shuffle=False, merge_last=True):
                # 如果有 godview 信息，则合并到 minibatch 中
                if 'god_view' in batch.data:
                    minibatch.god_view = batch.data['god_view'][minibatch.index]
    ...
    def learn(  # type: ignore
        ...
        for step in range(repeat):
            ...
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                value = self.critic(minibatch.obs, minibatch.god_view).flatten()
                if self._value_clip:
    ...
    def forward(self, batch: Batch, state: Optional[Union[dict, Batch, np.ndarray]] = None, **kwargs: Any) -> Batch:
        assert state is None, "State should be None for PPOPolicy."
        obs = batch.obs
        god_view = batch.get('god_view', None)
        actor_outputs = self.actor(obs, god_view)
        critic_outputs = self.critic(obs, god_view)
        return Batch(logits=actor_outputs.logits, act=actor_outputs.dist.sample().long(), state=state, dist=actor_outputs.dist, value=critic_outputs)