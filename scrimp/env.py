...
class TrackingEnv(gym.Env):
    ...
    def __init__(self, mission=0, base_obs_dim=12, use_god_view=False, god_view_dim=4):
        ...
        # 配置
        self.mask_flag = getattr(task_config, 'mask_flag', False)
    ...
    def _parse_actions(self, action: Union[int, Tuple[int, int], list, np.ndarray]):
        if action is None:
            return -1, -1
        if isinstance(action, (tuple, list, np.ndarray)) and len(action) == 2:
            ta, tg = int(action[0]), int(action[1])
            return ta, tg
        if isinstance(action, (int, np.integer)):
            return int(action), -1
        raise ValueError("Unsupported action format")

    def step(self, action: Union[int, Tuple[int, int], list, np.ndarray] = None, target_action: Optional[int] = None):
        self.step_count += 1

        if target_action is None:
            tracker_action, target_action = self._parse_actions(action)
        else:
            tracker_action, target_action = int(action), int(target_action)

        # Move tracker
        if tracker_action >= 0:
            self.tracker = utils.agent_move(self.tracker, tracker_action, self.tracker_speed)
        else:
            # 训练 target 时，rule-based tracker 也会移动
            self.tracker = utils.tracker_nav(self.tracker, self.target, self.tracker_speed)

        # Move target
        if target_action >= 0:
            self.target = utils.agent_move(self.target, target_action, self.target_speed)
        else:
            self.target = utils.target_nav(self.target, self.tracker, self.base, self.target_speed, self.target_frame_count)
        ...
        max_len = getattr(task_config, 'trail_max_len', 600)
        ...
        reward, terminated, truncated, info = utils.reward_calculate(self.tracker, self.target, self.base, mission=self.mission)
    ...
    def render(self, mode='human'):
        canvas = utils.get_canvas(self.target, self.tracker, self.base,
...
if __name__ == '__main__':
    ...
    obs, _ = env.reset()
    for _ in range(300):
        obs, r, ter, tru, info = env.step((np.random.randint(48), -1))
        if ter or tru:
            env.reset()
    env.close()
    sys.exit(0)